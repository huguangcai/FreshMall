...
public class GoodsFragment extends BaseFragment implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener {
    ...
    private ImageView img_title_right, img_search_left;
    ...
    protected void initData() {
        ...
        img_search_left = getViewById(R.id.img_search_left);
        ...
        mDataAdapter = new GoodsFragmentAdapter(getContext());

        mRecyclerView.setLayoutManager(new GridLayoutManager(getActivity(), 2));
        mLuRecyclerViewAdapter = new LuRecyclerViewAdapter(mDataAdapter);
        mRecyclerView.setAdapter(mLuRecyclerViewAdapter);
        if ("1".equals(SpUtils.getSp(getActivity(), "useType"))) {
            mLuRecyclerViewAdapter.addHeaderView(new HomeGoodsBanner(getActivity()));
            mLuRecyclerViewAdapter.addHeaderView(new HomeGoodsFooter(getActivity()));
        } else {
            mLuRecyclerViewAdapter.addHeaderView(new HomeGoodsBanner(getActivity()));
            mLuRecyclerViewAdapter.addHeaderView(new HomeGoodsHeadView(getActivity()));
            mLuRecyclerViewAdapter.addHeaderView(new HomeGoodsFooter(getActivity()));
        }
        ...
        mRecyclerView.setOnLoadMoreListener(new OnLoadMoreListener() {
            ...
            public void onLoadMore() {
                if (homeBestMoreBean != null) {
                    if (page < homeBestMoreBean.getPages()) {
                        page++;
                        requestData();
                    } else {
                        //the end
    ...
    protected void initListenser() {
        ...
        img_search_left.setOnClickListener(this);
...
//                startActivity(LocationActivity.class);
            ...
            switch (msg.what) {
                ...
                case -3:
                    if (mSwipeRefreshLayout.isRefreshing()) {
                        ...
                        mRecyclerView.refreshComplete(homeBestMoreBean.getData().size());
    ...
    private void getData() {
        NetWork.getService(ImpService.class)
                .HomeBestMoreData(String.valueOf(page), SpUtils.getSp(getActivity(), "uid"))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<HomeBestMoreBean>() {
                    private HomeBestMoreBean homeBestMoreBean;

                    @Override
                    public void onCompleted() {
                        if (homeBestMoreBean.getCode() == 0) {
                            showData(homeBestMoreBean);
                            List<HomeBestMoreBean.DataBean> data = homeBestMoreBean.getData();
    ...
    private void showData(HomeBestMoreBean homeBestMoreBean) {
        this.homeBestMoreBean = homeBestMoreBean;
    }
